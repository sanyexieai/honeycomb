...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 设置测试工作空间
    let workspace_root = PathBuf::from("/mnt/code_disk/code/rust/honeycomb/workspace/tenants/local/users/default");
    ...
    for (id, tag) in manager.get_tags() {
        println!("  - {} ({}): {} = {:.2}", id, tag.dimension, tag.name, tag.value);
    }
    ...
    println!("查询 \"我需要生成复杂的代码\" 与 code_generation 工具的相似度: {:.2}", similarity);
    ...
    println!("查询 \"我需要生成复杂的代码\" 与 file_operations 工具的相似度: {:.2}", similarity);
