...
use crate::{TagVector, HybridAnalysisResult, MultimodalTagAnalysisResult, AnalysisType};
...
impl ABTestingManager {
    ...
    pub fn new(config: ABTestingConfig) -> Self {
        ...
        // 注册统计检验方法
        statistical_analyzer.significance_tests.insert(
            "t-test".to_string(),
            Box::new(TTest),
        );
        statistical_analyzer.significance_tests.insert(
            "mann-whitney".to_string(),
            Box::new(MannWhitneyTest),
        );
    ...
    pub fn create_experiment(&mut self, mut experiment: Experiment) -> Result<String, String> {
        ...
        if experiment.traffic_allocation.is_empty() {
            experiment.traffic_allocation = vec![1.0 / experiment.variants.len() as f64; experiment.variants.len()];
        }
    ...
    pub fn start_experiment(&mut self, experiment_id: &str) -> Result<(), String> {
        ...
        if !matches!(experiment.status, ExperimentStatus::Draft | ExperimentStatus::Paused) {
    ...
    /// 分配用户到实验变体
    pub fn assign_variant(&self, experiment_id: &str, user_id: Option<&str>) -> Result<String, String> {
    ...
    pub fn analyze_experiment(&self, experiment_id: &str) -> Result<ExperimentReport, String> {
        ...
        // 生成结果摘要
        let results_summary = self.generate_results_summary(experiment, &variant_data, &statistical_tests);
    ...
    fn hash_assignment(&self, user_id: Option<&str>, experiment_id: &str, variant_count: usize) -> usize {
        ...
        // 简化的哈希函数
        let hash = hash_input.bytes().fold(0u64, |acc, b| {
    ...
    fn weighted_assignment(&self, user_id: Option<&str>, experiment_id: &str, weights: &[f64]) -> usize {
        let hash_input = format!("{}:{}", 
            user_id.unwrap_or("anonymous"), 
            experiment_id
        );
    ...
    fn update_aggregated_results(&mut self, data_point: &ExperimentDataPoint) {
        ...
        let variant_results = self.results_store.aggregated_results
            .entry(key)
        ...
        for (metric_name, &value) in &data_point.metrics {
            let stats = variant_results.metrics
    ...
    fn collect_variant_data(&self, experiment_id: &str) -> Result<HashMap<String, Vec<f64>>, String> {
        let experiment = self.experiments.get(experiment_id)
            .ok_or_else(|| format!("实验不存在: {}", experiment_id))?;
    ...
    fn perform_statistical_tests(
        ...
        let control_data = variant_data.get(&control_variant.id)
            .ok_or("控制组无数据")?;
                ...
                if let Some(treatment_data) = variant_data.get(&variant.id) {
                    if !treatment_data.is_empty() {
                        ...
                        results.push(StatisticalTestResult {
                            ...
                            metric_name: experiment.metrics.iter()
                                .find(|m| m.is_primary)
                                .unwrap()
                                .name.clone(),
                        ...
                        let mann_whitney = &self.statistical_analyzer.significance_tests["mann-whitney"];
    ...
    fn generate_results_summary(
        ...
        for variant in &experiment.variants {
            if let Some(data) = variant_data.get(&variant.id) {
                ...
                variant_performance.insert(variant.id.clone(), VariantPerformance {
        ...
        let winning_variant = variant_performance.iter()
            .max_by(|a, b| a.1.primary_metric_value.partial_cmp(&b.1.primary_metric_value).unwrap())
            .map(|(id, _)| id.clone());
        ...
        let performance_improvement = if let Some(winner_id) = &winning_variant {
            if winner_id != &control_variant.id {
                let control_perf = variant_performance.get(&control_variant.id)
                    .map(|p| p.primary_metric_value)
                    .unwrap_or(0.0);
                let winner_perf = variant_performance.get(winner_id)
                    .map(|p| p.primary_metric_value)
                    .unwrap_or(0.0);
    ...
    fn generate_recommendations(
        ...
        if has_significant {
            ...
            let best_test = statistical_tests.iter()
                .filter(|t| t.is_significant)
                .max_by(|a, b| a.effect_size.abs().partial_cmp(&b.effect_size.abs()).unwrap());
...
mod tests {
    ...
    fn test_variant_assignment() {
        ...
        // 测试相同用户的一致性分配
        let variant1 = manager.assign_variant(&experiment_id, Some("user1")).unwrap();
        let variant2 = manager.assign_variant(&experiment_id, Some("user1")).unwrap();
        assert_eq!(variant1, variant2);
        
        // 测试不同用户可能有不同分配
        let variant3 = manager.assign_variant(&experiment_id, Some("user2")).unwrap();
