...
pub use ab_testing::{
    ABTestingConfig, ABTestingManager, Experiment, ExperimentDataPoint, ExperimentReport,
    ExperimentStatus, ExperimentVariant, MetricDefinition, MetricType, SignificanceTestResult,
};
pub use context::{
    ContextAnalysisResult, ContextAwareAnalyzer, ContextConfig, ConversationTurn, ExtendedContext,
    SessionInfo, TaskRecord,
};
pub use dynamic_learning::{
    ComponentWeights, DynamicLearningConfig, DynamicLearningManager,
    FeedbackType as DynamicFeedbackType, LearningResult, LearningStatistics, ProcessedFeedback,
};
pub use explainable::{
    AlternativePossibility, ClassificationExplanation, ConfidenceBreakdown, ContributingFactor,
    DecisionPath, ExplainableAnalyzer,
};
pub use fuzzy_matcher::{FuzzyMatcher, FuzzyMatcherConfig, MatchResult, MatchType};
pub use hierarchical_intent::{
    ClassificationLevel, HierarchicalConfig, HierarchicalIntentClassifier, HierarchicalResult,
    HierarchyStats, IntentCandidate, IntentNode,
};
pub use multimodal::{
    AudioFeatures, AudioMetadata, DocumentMetadata, ImageMetadata, MultimodalAnalysisManager,
    MultimodalAnalysisResult, MultimodalConfig, MultimodalInput, MultimodalProcessingBackend,
    VideoMetadata, VisualFeatures,
};
pub use multipath::{
    FusionStrategy, MatchContext, MatchPathType, MultiPathConfig, MultiPathMatcher,
    MultiPathResult, PathMatchResult,
};
pub use performance::{
    AnalysisType, CacheConfig, CacheStats, IncrementalUpdater, PerformanceOptimizer,
};
pub use personalization::{
    FeedbackType as PersonalizationFeedbackType, PersonalizationConfig, PersonalizationManager,
    PersonalizedResult, UserFeedback, UserPreferences, UserProfile, UserStatistics,
};
pub use reinforcement_learning::{
    Action as RLAction, ActionRecommendation, Experience, RLAlgorithm, RLConfig, RLStatistics,
    ReinforcementLearningManager, State as RLState,
};
pub use vector_matching::{
    CacheInfo, KeywordMatch, ModelType, SemanticContext, VectorMatchResult, VectorMatcher,
    VectorMatcherConfig,
};
...
impl TagVector {
    ...
    pub fn with_dimension(mut self, dimension: &str, value: f32) -> Self {
        self.dimensions
            .insert(dimension.to_string(), value.clamp(0.0, 1.0));
        self
    }
...
impl TagSystemManager {
    ...
    pub fn with_full_config(
        ...
        let vector_matcher = vector_config.and_then(|config| VectorMatcher::new(config).ok());

        let hierarchical_classifier =
            hierarchical_config.map(|config| HierarchicalIntentClassifier::new(config));

        let dynamic_learning_manager = learning_config
            .map(|config| DynamicLearningManager::new(workspace_root.clone(), config));

        let rl_manager = rl_config
            .map(|config| ReinforcementLearningManager::new(workspace_root.clone(), config));

        let multimodal_manager = multimodal_config
            .map(|config| MultimodalAnalysisManager::new(workspace_root.clone(), config));
        ...
        Self {
            ...
            personalization_manager: Some(PersonalizationManager::new(
                workspace_root,
    ...
    fn parse_dimension_room(&self, path: &Path) -> Result<Dimension> {
        ...
        let dimension = Dimension {
            ...
            description: frontmatter
                ...
                .clone(),
            scale_min: frontmatter
                .get("scale_min")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.0),
            scale_max: frontmatter
                .get("scale_max")
                .and_then(|s| s.parse().ok())
                .unwrap_or(1.0),
    ...
    fn parse_tag_room(&self, path: &Path) -> Result<Tag> {
        ...
        let tag = Tag {
            ...
            compatible_weight: frontmatter
                .get("compatible_weight")
                .and_then(|s| s.parse().ok())
                .unwrap_or(1.0),
        };
    ...
    fn load_entity_histories(&mut self) -> Result<()> {
        ...
        for entity_type_entry in fs::read_dir(&entities_dir)? {
            ...
            if entity_type_path.is_dir() {
                let entity_type = entity_type_path
                    .file_name()
                    .and_then(|s| s.to_str())
                    .unwrap_or("unknown")
                    .to_string();
    ...
    /// 带缓存的分析方法
    pub fn analyze_input_tags_cached(
        &mut self,
        input: &str,
        ...
        if let Some(cached_result) = self
            ...
            .get_cached_result(input, analysis_type.clone())
        {
        ...
        self.performance_optimizer
            .cache_result(input, analysis_type, result.clone());
        result
    }
        ...
        // 先更新内存中的数据
        {
            let history =
                self.entity_histories
                    .entry(key.clone())
                    .or_insert_with(|| EntityTagHistory {
    ...
    pub fn warmup_cache(&mut self) {
        ...
        self.performance_optimizer
            .precompute_common_matches(&keywords_map);
    }
    ...
    /// 分析输入并生成完整的可解释性报告
    pub fn analyze_with_explanation(
        &self,
        input: &str,
    ) -> (TagAnalysisResult, ClassificationExplanation) {
        let analysis_result = self.analyze_with_details(input);
        let explanation =
            ExplainableAnalyzer::generate_explanation(input, &analysis_result.dimension_details);
    ...
    pub fn quick_explain(&mut self, input: &str) -> ClassificationExplanation {
        // 先尝试使用详细分析的缓存结果
        let analysis_result = if let Some(cached) = self
            .performance_optimizer
            .get_cached_result(input, AnalysisType::Detailed)
        {
            TagAnalysisResult {
                tag_vector: cached,
    ...
    ) -> Option<MultiPathResult> {
        self.multipath_matcher
            .as_ref()
            .map(|matcher| matcher.match_input(input, context))
    }

    /// 使用多路匹配分析并返回最终标签向量
    pub fn analyze_input_tags_multipath(
        &self,
        input: &str,
        context: Option<&MatchContext>,
    ) -> TagVector {
    ...
    /// 比较不同方法的分析结果
    pub fn compare_analysis_methods(
        &mut self,
        input: &str,
        context: Option<&MatchContext>,
    ) -> AnalysisComparison {
        ...
        // 缓存结果用于后续比较
        self.performance_optimizer
            .cache_result(input, AnalysisType::Legacy, legacy_result.clone());
        self.performance_optimizer.cache_result(
            input,
            AnalysisType::Enhanced,
            enhanced_result.clone(),
        );

        let similarities =
            self.calculate_method_similarities(&legacy_result, &enhanced_result, &multipath_result);
        let recommendation =
            self.recommend_best_method(&legacy_result, &enhanced_result, &multipath_result);
    ...
    fn calculate_method_score(&self, tag_vector: &TagVector) -> f32 {
        ...
        let avg_value: f32 =
            tag_vector.dimensions.values().sum::<f32>() / tag_vector.dimensions.len().max(1) as f32;
    ...
    pub fn analyze_with_extended_context(
        ...
        let improvement_score = context_analysis
            .as_ref()
    ...
    pub fn analyze_intelligently(
        ...
        // 选择最佳结果
        let best_method = analysis_results
            .iter()
        ...
        extended_context
            ...
            .push(ConversationTurn {
                ...
                assistant_response: format!(
                    "分析完成，结果: {:?}",
                    result.dimensions.keys().collect::<Vec<_>>()
                ),
    ...
    /// 启用个性化功能
    pub fn enable_personalization(
        &mut self,
        config: PersonalizationConfig,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mut personalization_manager =
            PersonalizationManager::new(self.workspace_root.clone(), config);
    ...
    pub fn analyze_personalized(
        ...
        let confidence_score = personalized
            .as_ref()
    ...
    pub fn process_user_feedback(
        ...
        if let Some(pm) = &mut self.personalization_manager {
            let process_result =
                pm.process_feedback(user_id, input, predicted_result, feedback.clone());
    ...
    pub fn get_user_insights(&self, user_id: &str) -> Option<UserInsights> {
        self.personalization_manager.as_ref().and_then(|pm| {
            pm.get_user_statistics(user_id).map(|stats| UserInsights {
                user_id: user_id.to_string(),
                ...
                strengths: vec![if stats.success_rate > 0.8 {
                    "分析准确率较高".to_string()
                } else {
    ...
    pub fn analyze_input_tags_vector(&self, input: &str) -> Result<VectorAnalysisResult, String> {
        let vector_matcher = self.vector_matcher.as_ref().ok_or("向量匹配功能未启用")?;

        let vector_results = vector_matcher
            .vector_match(input, &self.dimensions)
            .map_err(|e| format!("向量匹配失败: {}", e))?;

        let tag_vector =
            vector_matcher.vector_results_to_tag_vector(&vector_results, &self.dimensions);
    ...
    fn fuse_hybrid_results(
        ...
        for dim in legacy.dimensions.keys() {
            all_dimensions.insert(dim.clone());
        }
        for dim in enhanced.dimensions.keys() {
            all_dimensions.insert(dim.clone());
        }
        if let Some(v) = vector {
            for dim in v.dimensions.keys() {
                all_dimensions.insert(dim.clone());
            }
        }
    ...
    fn determine_fusion_strategy(&self) -> String {
        ...
        if self.vector_matcher.is_some() {
            strategies.push("向量匹配");
        }
        if self.multipath_matcher.is_some() {
            strategies.push("多路径");
        }
        if self.personalization_manager.is_some() {
            strategies.push("个性化");
        }
    ...
    pub fn analyze_with_intent(
        ...
        let confidence_score =
            self.calculate_intent_aware_confidence(&intent_result, &adjusted_tags);
    ...
    /// 根据意图调整标签权重
    fn adjust_tags_by_intent(
        &self,
        tags: &TagVector,
        intent_result: &HierarchicalResult,
    ) -> TagVector {
    ...
    /// 生成基于意图的建议
    fn generate_intent_based_suggestions(
        &self,
        intent_result: &Option<HierarchicalResult>,
        tags: &TagVector,
    ) -> Vec<String> {
        ...
        if let Some(intent) = intent_result {
            if let Some(final_intent) = &intent.final_intent {
                match final_intent.as_str() {
                    ...
                    id if id.contains("knowledge_query") => {
                        suggestions
                            .push("建议提供更具体的查询上下文以获得更准确的结果".to_string());
                    }
            ...
            if !intent.alternative_paths.is_empty() {
                suggestions.push(format!(
                    "检测到{}个可能的替代意图，建议进一步澄清",
                    intent.alternative_paths.len()
                ));
            }
        } else {
    ...
    /// 生成分析洞察
    fn generate_analysis_insights(
        &self,
        intent_result: &Option<HierarchicalResult>,
        tag_result: &HybridAnalysisResult,
    ) -> Vec<String> {
        ...
        if let Some(intent) = intent_result {
            insights.push(format!(
                "意图分类经过{}个层级，最终置信度{:.1}%",
                intent.classification_path.len(),
                intent.overall_confidence * 100.0
            ));
        ...
        insights.push(format!(
            "标签分析使用了{}策略，置信度{:.1}%",
            tag_result.fusion_strategy,
            tag_result.confidence_score * 100.0
        ));

        // 性能洞察
        let total_time = intent_result
            .as_ref()
            .map(|i| i.processing_time)
            .unwrap_or_default()
            + tag_result.analysis_duration;
        insights.push(format!(
            "总分析耗时{:.1}ms",
    ...
    /// 计算意图感知的置信度
    fn calculate_intent_aware_confidence(
        &self,
        intent_result: &Option<HierarchicalResult>,
        tags: &TagVector,
    ) -> f32 {
        let intent_confidence = intent_result
            .as_ref()
    ...
    /// 使用动态权重执行分析
    fn analyze_with_dynamic_weights(
        &mut self,
        input: &str,
        user_id: Option<&str>,
    ) -> HybridAnalysisResult {
    ...
    fn analyze_with_custom_weights(
        ...
        HybridAnalysisResult {
            ...
            multipath_result,
            personalized_result,
            fusion_strategy: format!(
                "动态权重融合 (L:{:.2} E:{:.2} V:{:.2} M:{:.2} P:{:.2})",
    ...
    pub fn analyze_with_reinforcement_learning(
        ...
        if let Some(rl_manager) = &mut self.rl_manager {
            ...
            let reward =
                rl_manager.calculate_reward(&analysis_result, user_satisfaction, response_time);
    ...
    pub fn train_rl_agent(&mut self, training_inputs: Vec<TrainingExample>) -> TrainingResult {
        ...
        for example in &training_inputs {
            ...
            let (state, action) = {
                ...
                let state =
                    rl_manager.build_state_from_input(&example.input, example.user_id.as_deref());
                let action = rl_manager.select_action(&state);
                (state, action)
            };
            ...
            // 第二步：执行分析
            let weights = action.to_component_weights();
            let analysis_result = self.analyze_with_custom_weights(
                &example.input,
                example.user_id.as_deref(),
                &weights,
            );
            ...
            {
                let rl_manager = self.rl_manager.as_mut().unwrap();
                let reward = rl_manager.calculate_reward(
                    &analysis_result,
                    example.expected_satisfaction,
        ...
        let episode = self.rl_manager.as_mut().unwrap().end_episode(
            episode_id,
            experiences,
            training_duration,
        );
    ...
    pub fn get_rl_statistics(&self) -> Option<RLStatistics> {
        self.rl_manager
            .as_ref()
            .map(|rl: &ReinforcementLearningManager| rl.get_rl_statistics())
    }
    ...
    pub fn get_action_recommendation(
        ...
        self.rl_manager
            .as_ref()
            .map(|rl: &ReinforcementLearningManager| {
    ...
    pub fn enable_multimodal_analysis(&mut self, config: MultimodalConfig) -> Result<(), String> {
        let multimodal_manager =
            MultimodalAnalysisManager::new(self.workspace_root.clone(), config);
        self.multimodal_manager = Some(multimodal_manager);
        Ok(())
    }

    /// 分析多模态输入
    pub fn analyze_multimodal_input(
        &self,
        input: &MultimodalInput,
    ) -> Result<MultimodalTagAnalysisResult, String> {
        let start_time = std::time::Instant::now();
        ...
        if let Some(multimodal_manager) = &self.multimodal_manager {
            ...
            // 3. 融合多模态特征和文本分析结果
            let fused_tags =
                self.fuse_multimodal_and_text_analysis(&multimodal_result, text_analysis.as_ref());

            // 4. 计算综合置信度
            let overall_confidence =
                self.calculate_multimodal_confidence(&multimodal_result, text_analysis.as_ref());
    ...
    /// 批量分析多模态输入
    pub fn batch_analyze_multimodal(
        &self,
        inputs: &[MultimodalInput],
    ) -> Result<Vec<MultimodalTagAnalysisResult>, String> {
    ...
    /// 生成多模态分析洞察
    fn generate_multimodal_insights(
        &self,
        input: &MultimodalInput,
        tags: &TagVector,
    ) -> Vec<String> {
        ...
        match input {
            MultimodalInput::Image { metadata, .. } => {
                ...
                insights.push(format!(
                    "图像亮度: {:.2}, 对比度: {:.2}",
                    metadata.brightness, metadata.contrast
                ));
            }
    ...
    /// 生成多模态分析建议
    fn generate_multimodal_recommendations(
        &self,
        input: &MultimodalInput,
        tags: &TagVector,
    ) -> Vec<String> {
    ...
    pub fn get_multimodal_supported_formats(&self) -> Option<HashMap<String, Vec<String>>> {
        self.multimodal_manager
            .as_ref()
            .map(|manager| manager.get_supported_formats())
    }
    ...
    pub fn batch_analyze_with_ab_testing(
        ...
        for (input, user_id) in inputs {
            match self.analyze_with_ab_testing(input, experiment_id, user_id.as_deref()) {
                Ok(result) => results.push(result),
                Err(e) => {
    ...
    /// 获取A/B测试实验状态
    pub fn get_ab_experiment_status(
        &self,
        experiment_id: &str,
        ...
        for (algorithm_name, config) in algorithm_configs {
            ...
            for input in test_inputs {
                ...
                let result = match config {
                    AlgorithmVariant::Baseline => self.analyze_input_tags(input),
                    AlgorithmVariant::Enhanced => self.analyze_input_tags(input),
        ...
        let analysis_result = match &variant.algorithm_config {
            ...
            ab_testing::AlgorithmConfig::MultimodalAnalysis(_config) => {
                if let Some(_) = &self.multimodal_manager {
                    let multimodal_input =
                        crate::multimodal::MultimodalInput::Text(input.to_string());
                    match self.analyze_multimodal_input(&multimodal_input) {
    ...
    fn create_experiment_data_point(
        ...
        let (tag_vector, confidence_score) = match analysis_result {
            ...
            VariantAnalysisResult::Hybrid(hybrid_result) => (
                hybrid_result.final_result.clone(),
                hybrid_result.confidence_score as f64,
            ),
            VariantAnalysisResult::Multimodal(multimodal_result) => (
                multimodal_result.fused_tags.clone(),
                multimodal_result.overall_confidence as f64,
            ),
            VariantAnalysisResult::ReinforcementLearning(rl_result) => {
                // 使用RL结果的整体表现作为置信度
                let confidence = rl_result
                    .rl_statistics
        ...
        ab_testing::ExperimentDataPoint {
            ...
            analysis_result: ab_testing::AnalysisResultSummary {
                ...
                analysis_type: match analysis_result {
                    ...
                    VariantAnalysisResult::ReinforcementLearning(_) => {
    ...
    fn calculate_basic_confidence(&self, tags: &TagVector) -> f64 {
        if tags.dimensions.is_empty() {
            ...
            let avg_confidence =
                tags.dimensions.values().sum::<f32>() / tags.dimensions.len() as f32;
    ...
    fn generate_comparison_summary(
        ...
        ComparisonSummary {
            ...
            average_processing_time: results
                .values()
                .map(|perf| perf.average_processing_time)
                .sum::<std::time::Duration>()
                / total_algorithms as u32,
        }
    }
    ...
    fn generate_algorithm_recommendations(
        ...
        // 性能 vs 准确性权衡分析
        let performance_scores: Vec<_> = results
            .iter()
            .map(|(name, perf)| {
        ...
        if let Some((best_algorithm, _)) = performance_scores
            .iter()
            .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        {
        ...
        if let Some((most_accurate, _)) = results.iter().max_by(|(_, perf_a), (_, perf_b)| {
...
fn format_entity_tags_room(history: &EntityTagHistory) -> Result<String> {
    ...
    if !history.history.is_empty() {
        content.push_str("\n## 标签变更历史\n\n");
        for entry in history.history.iter().take(5) {
            // 只显示最近5条
            content.push_str(&format!(
                "- {} ({}): 触发原因: {}\n",
...
---"#;
        ...
        fs::write(
            dimensions_dir.join("creativity_level.md"),
            creativity_content,
        )
        .unwrap();
...
---"#;
        ...
        // 结果应该一致
        assert_eq!(
            result1.get("creativity_level"),
            result2.get("creativity_level")
        );
...
---"#;
        ...
        // 验证贡献因子
        let has_exact_match = explanation.contributing_factors.iter().any(|f| {
            matches!(
                f.factor_type,
                crate::explainable::FactorType::ExactKeywordMatch
            )
        });
        ...
        println!(
            "整体置信度: {:.1}%",
            explanation.confidence_breakdown.overall_confidence * 100.0
        );
    }
...
---"#;
        ...
        // 验证包含不同类型的匹配器
        let path_types: std::collections::HashSet<MatchPathType> = result
            .path_results
            .iter()
        ...
        context
            .user_preferences
            .insert("creativity_level".to_string(), 0.9);
...
---"#;
        ...
        assert_eq!(
            extended_context.conversation_history.len(),
        ...
        println!(
            "方法置信度: {:.1}%",
            intelligent_result.method_confidence * 100.0
        );
        println!(
            "上下文改进分数: {:.1}%",
            context_result.improvement_score * 100.0
        );
    }
...
---"#;
        ...
        assert!(
            manager
                .enable_personalization(PersonalizationConfig::default())
                .is_ok()
        );
        ...
        let feedback_result =
            manager.process_user_feedback(user_id, input, &result.final_result, feedback);
        ...
        // 再次分析，应该应用学习到的偏好
        let second_analysis =
            manager.analyze_personalized(user_id, "design innovative solutions", None);
        ...
        println!(
            "反馈处理结果: 学习={}, 适应={}",
            fb_result.learning_applied, fb_result.adaptation_applied
        );
...
---"#;
        ...
        println!(
            "  创造性分数: {:.3}",
            result.tag_vector.get("creativity_level")
        );
        println!(
            "  匹配关键词数: {}",
            creativity_match.matched_keywords.len()
        );
        println!(
            "  语义密度: {:.3}",
            creativity_match.semantic_context.semantic_density
        );
...
---"#;
        ...
        if let Some(vector_result) = &hybrid_result.vector_result {
            println!(
                "向量结果维度数: {}",
                vector_result.tag_vector.dimensions.len()
            );
        ...
        if let Some(multipath_result) = &hybrid_result.multipath_result {
            println!(
                "多路径结果维度数: {}",
                multipath_result.final_tag_vector.dimensions.len()
            );
        ...
        println!(
            "  最终分数: {:.3}",
            hybrid_result.final_result.get("creativity_level")
        );
        println!(
            "  传统方法: {:.3}",
            hybrid_result.legacy_result.get("creativity_level")
        );
        println!(
            "  增强方法: {:.3}",
            hybrid_result.enhanced_result.get("creativity_level")
        );

        if let Some(vector_result) = &hybrid_result.vector_result {
            println!(
                "  向量方法: {:.3}",
                vector_result.tag_vector.get("creativity_level")
            );
        }
    }
...
---"#;
        ...
        assert!(
            manager
                .enable_hierarchical_intent(hierarchical_config)
                .is_ok()
        );
        ...
        for level in &intent_result.classification_path {
            ...
            for candidate in &level.candidates {
                println!(
                    "    候选: {} ({:.3})",
                    candidate.intent_id, candidate.confidence
                );
            }
        }
...
---"#;
        ...
        assert!(
            manager
                .enable_vector_matching(VectorMatcherConfig::default())
                .is_ok()
        );
        assert!(
            manager
                .enable_hierarchical_intent(HierarchicalConfig::default())
                .is_ok()
        );
...
---"#;
        ...
        if let Some(weights) = &result.weights_optimization {
            ...
            println!(
                "    多路径: {:.3}",
                weights.current_weights.multipath_weight
            );
            println!(
                "    个性化: {:.3}",
                weights.current_weights.personalized_weight
            );
...
---"#;
        ...
        assert!(
            manager
                .enable_dynamic_learning(DynamicLearningConfig::default())
                .is_ok()
        );
        ...
        println!(
            "  用户满意度: {:.3}",
            stats.performance_metrics.user_satisfaction
        );
    }
    ...
    fn test_weight_optimization_scenarios() {
        ...
        for (input, expected_creativity, satisfaction) in &test_cases {
            ...
            println!(
                "  期望创造性: {:.2}, 满意度: {:.2}",
                expected_creativity, satisfaction
            );
        ...
        println!(
            "  最终准确度: {:.3}",
            final_stats.performance_metrics.accuracy
        );
        println!(
            "  最终满意度: {:.3}",
            final_stats.performance_metrics.user_satisfaction
        );
    }
...
---"#;
        ...
        let result =
            manager.analyze_with_reinforcement_learning(input, Some("test_user"), Some(0.8));
        ...
        println!(
            "  探索率: {:.3}",
            result.rl_statistics.current_exploration_rate
        );
        println!("  动作推荐: {}", result.action_recommendation.reasoning);
        println!(
            "  推荐置信度: {:.3}",
            result.action_recommendation.confidence
        );
...
---"#;
        ...
        println!(
            "  经验缓冲区大小: {}",
            training_result.rl_statistics.experience_buffer_size
        );
        println!(
            "  当前探索率: {:.3}",
            training_result.rl_statistics.current_exploration_rate
        );
        ...
        println!(
            "  调整后探索率: {:.3}",
            stats_after_adjustment.current_exploration_rate
        );
    }
    ...
    fn test_rl_action_strategies() {
        ...
        assert!(
            manager
                .enable_reinforcement_learning(RLConfig::default())
                .is_ok()
        );
        ...
        for (input, task_type) in &test_cases {
            let result =
                manager.analyze_with_reinforcement_learning(input, Some("test_user"), Some(0.7));
        ...
        println!(
            "  平均episode奖励: {:.3}",
            final_stats.average_episode_reward
        );
        println!("  Q表大小: {}", final_stats.q_table_size);
        println!(
            "  奖励方差: {:.3}",
            final_stats.convergence_metrics.reward_variance
        );
        println!(
            "  策略稳定性: {:.3}",
            final_stats.convergence_metrics.policy_stability
        );
    }
    ...
    fn test_intent_specific_scenarios() {
        ...
        for (input, expected_category) in test_cases {
            ...
            if let Some(level1) = result.classification_path.get(0) {
                ...
                // 验证一级分类包含期望的类别
                assert!(
                    level1.predicted_intent.contains(expected_category)
                        || level1
                            .candidates
                            .iter()
                            .any(|c| c.intent_id.contains(expected_category)),
                    "期望分类 '{}' 但得到 '{}'",
                    expected_category,
                    level1.predicted_intent
                );
            }
    ...
    fn test_multimodal_analysis_integration() {
        ...
        assert!(
            manager
                .enable_multimodal_analysis(multimodal_config)
                .is_ok()
        );
    ...
    fn test_batch_multimodal_analysis() {
        ...
        assert!(
            manager
                .enable_multimodal_analysis(MultimodalConfig::default())
                .is_ok()
        );
    ...
    fn test_smart_multimodal_analysis() {
        ...
        println!(
            "智能分析结果 final_tags: {:?}",
            smart_result.final_tags.dimensions
        );
    ...
    fn test_algorithm_comparison() {
        ...
        println!(
            "  最准确算法: {:?}",
            result.test_summary.most_accurate_algorithm
        );
        println!(
            "  平均处理时间: {:?}",
            result.test_summary.average_processing_time
        );

        for (algorithm, performance) in &result.comparison_results {
            println!(
                "  {} - 平均时间: {:?}",
                algorithm, performance.average_processing_time
            );
        }
    ...
    fn test_ab_experiment_lifecycle() {
        ...
        assert!(
            manager
                .enable_ab_testing(ABTestingConfig::default())
                .is_ok()
        );
