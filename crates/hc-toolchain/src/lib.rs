...
mod tests {
    use super::{
        ToolExecutionKind, ToolExecutionOutcome, ToolProvider, ToolRepository, ToolSpec,
        ToolStability, build_default_tool_execution_plan, builtin_tool, default_tool_catalog,
        default_tool_command, seed_tool_local_dir_list, seed_tool_local_file_read, seed_tool_rg,
    };
    use hc_capability::ModelDependence;
    ...
    fn builtin_tool_lookup_returns_registered_tool() {
        let tool =
            builtin_tool("tool.local-file.read").expect("builtin local file read should exist");
    ...
    fn default_plan_supports_core_local_file_tools() {
        ...
        let plan =
            build_default_tool_execution_plan(&tool, "read README").expect("plan should build");
