...
>
> **「都做完」指什么**：**Part A** 下方仍 **`[ ]` 的条目**（主要是 **HTTP chat 与 workbench 的 L2/L3 编排对等**、**consolidate / 结构化工件链**、**隐式工单非退化执行**）属于 **P0 主路径尚未收口的能力**；**Part B（ADR-006）** 为 **经验副链**，单独排期，**默认不阻塞** Part A 交付。把 Part B 全勾掉需要另一条产品线级别的实现，不是 Timed/Scheduler 同一量级。
...
- [x] **检索裁剪**：若请求未显式传 `memory.scope`，`hc-service` 的 chat 准备路径与 swarm 路由/绑定共用同一次分类；**`L1` → `session` 近窗语义**；**`L2/L3`** 在有 task 锚时 → **`task` scope + `room_anchor`**——锚含 **请求/会话 `active_task_id`**、task-layer **`room_id` 推导**、以及 **同轮 HTTP 隐式分配的 `task.http.implicit.*`**（[`prepare_chat_request`](../../crates/runtime/hc-service/src/chat.rs) 在 `emit_swarm_observability_from_classified` 之后并入 memory hint）；**无锚**时退化为 `session`（避免扩大跨 task 召回）。客户端显式 `memory.scope` 时仍优先生效。
...
- [x] **`Turn` 路由单次 classify**：[`try_handle_service_turn`](../../crates/runtime/hc-service/src/turn.rs)（同步 `/ turn` outcome）与 **`handle_turn_stream_request`**（流式）共用 **`resolve_turn_fallback_room_attachment`** + **`classify_turn_route_decision`** + **`emit_turn_route_swarm_observability_non_chat_fallback`**，保证单次 **`TurnProviderRegistry::decide`**；stream 仍为 **room attach → `Started` → decide**。回归：[`shared_classifier_plain_message_chat_fallback_then_try_handle_aligned`](../../crates/runtime/hc-service/src/turn.rs)（ChatFallback）、[`classify_turn_cn_reminder_selects_timed_provider`](../../crates/runtime/hc-service/src/turn.rs)（中文提醒 → **`Timed`**）、[`classify_turn_cn_reminder_room_disables_timed_selects_chat_fallback`](../../crates/runtime/hc-service/src/turn.rs)（房间 **`enabled_providers` 不含 `timed`** → ChatFallback）、[`classify_turn_pending_confirmation_wins_when_session_awaits_confirmation`](../../crates/runtime/hc-service/src/turn.rs)（会话 **`pending_confirmation`** + 确认话术 → **`pending_confirmation`**；`tool-routing-tags.md` 含 **`id`/`type`**；房间 stub 关 **`mcp_tool`**）、[`classify_turn_cn_confirm_room_disables_pending_selects_chat_fallback`](../../crates/runtime/hc-service/src/turn.rs)（同上 pending 会话 + 确认话术，但房间 stub 关掉 **`pending_confirmation`**（及 **`mcp_tool`**）→ ChatFallback）。

---

## Phase 4：Bootstrap 与 planner_only（ADR-002）

- [x] **`bootstrap`**：**`bootstrap_task` 默认**为 **planner_only**（等价于 `bootstrap_planning_task` / 单 `planner` seed）。**三角色**（planner + worker + reviewer）通过显式 API [`bootstrap_task_with_preset`](../../crates/agent/hc-agent/src/bootstrap.rs)（`TaskBootstrapPreset::ThreeRolesDemo`）或环境变量 **`HC_TASK_BOOTSTRAP_PRESET=three_roles`**（别名 `three-roles`、`demo`、`full`）启用，供 demo / 单测。详见 `hc-agent::bootstrap`。
- [x] **Materialization 上限**：环境变量 **`HC_MAX_AGENTS_PER_TASK`**、**`HC_MAX_NEW_AGENTS_PER_ROUND`**（正整数；未设置或无效视为该维度不限制）——[`materialize_plan`](../../crates/agent/hc-agent/src/bootstrap.rs) 对每个 `AgentPlan` 取 **min(seeds 数, 两上限)**，只物化 **前缀 seeds**；触顶 **`tracing::warn`** + [`MaterializePlanOutcome::notices`](../../crates/agent/hc-agent/src/bootstrap.rs)。[`bootstrap_task_workbench`](../../crates/agent/hc-agent/src/workbench.rs) 在有 notice 时追加 **`coordination/<slug>/materialization_notices.jsonl`**（`materialization_notice_v1`）。另有 [`materialize_plan_with_limits`](../../crates/agent/hc-agent/src/bootstrap.rs) 供测试显式传 [`MaterializePlanLimits`](../../crates/agent/hc-agent/src/bootstrap.rs)。**跨轮累计**同一 task 已存活实例数的硬会计仍待编排状态；当前为 **单次 batch / 单 plan** 截断。
...
- [x] **`L1`**：保留现有 **message-level nomination → reply**——**工作台 / `hc-agent` headless**：`AgentOrchestrator::run_nomination_cycle`（[`orchestrator.rs`](../../crates/agent/hc-agent/src/orchestrator.rs)）对 **`RoutingTier::L1`** 的 Chat 消息走 **`suggest_claims_for_message` → `ResolveSpeakingGrant` → `generate_and_post_reply`**；**`L2`/`L3`** 同路径下 **`message_nomination_skipped_for_routing_tier`**（`grant: None`，打 `hc_trace`）。——**HTTP `hc-service` Chat** 仍为 **`route_agent` + 单次 LLM 生成**（无双实例 nomination）；与上文 **渐进迁移** 并存，P0 不强制远端与工作台完全一致。
- [x] **`L2/L3`（工作台 / headless）**：主路径为 **TaskPlan → WorkItem → claim → assign → 执行**；**不以 message-level nomination 决定 work item owner**。**工作台 / headless runtime** 在 L2/L3 已跳过 message nomination，走 task 侧编排（与 Phase 5 **`L1`** 条目中的 `message_nomination_skipped_for_routing_tier` 描述一致）。
- [ ] **`L2/L3`（HTTP `hc-service` chat 对等）**：仍为 **`route_agent` + 单次生成**（无双实例 nomination、无完整编排状态机）。**已具备（P0 渐进）**：当有 **task 锚**（含 **`task.http.implicit.*`**、绑定/请求 **`active_task_id`**、task-layer **`room_id` 推导**）且工作区已有 **`coordination/<slug>/task_plan.md`** 时，[`prepare_chat_request`](../../crates/runtime/hc-service/src/chat.rs) 将其正文 **节选**并入 **system prompt**，只读（回归：[`l23_reuse_active_task_appends_task_plan_excerpt_and_speaker_appendix`](../../crates/runtime/hc-service/src/chat.rs)、[`l1_with_task_on_disk_does_not_append_task_plan_excerpt`](../../crates/runtime/hc-service/src/chat.rs)、[`l23_http_implicit_task_anchor_appends_task_plan_excerpt_when_file_preseeded`](../../crates/runtime/hc-service/src/chat.rs)，隐式 slug 与 [`prepare_chat_request_with_swarm_clock`](../../crates/runtime/hc-service/src/chat.rs) 的 **`swarm_created_at_ms`** 对齐）。L2/L3 下另经 [`maybe_persist_http_chat_l23_degenerate_claim_assign`](../../crates/runtime/hc-service/src/chat.rs) 落 **退化** claim/assignment（[`persist_http_chat_l23_degenerate_claim_assign`](../../crates/agent/hc-agent/src/persistence.rs)，单测：`hc-agent/tests/unit/persistence.rs`）。**可选（实验）**：**`HC_HTTP_CHAT_L2L3_PLANNER_STEER=1`** 时，在同轮 excerpt 之前调用 [`maybe_apply_http_l2l3_planner_steering`](../../crates/agent/hc-agent/src/http_l2l3_planner_steering.rs)，用与工作台相同的 planner 提示模板写回 **结构化 JSON draft** 到磁盘 `task_plan`（额外 LLM 调用；不替代完整编排状态机）。**仍缺**：**完整**非退化执行编排及与 **工作台 / headless** 的 **行为对等**（与下条「隐式工单后续」、Phase 7 **consolidate** 同轨收口）。
- [x] **`L2` 隐式工单（HTTP P0：创建、绑定、幂等、stub）**：无 task 锚的 L2/L3 轮次由 [`emit_swarm_observability_from_classified`](../../crates/runtime/hc-service/src/chat.rs) 分配 **`task.http.implicit.{wall_clock_ms}`**（与同轮 **`{message_id_prefix}.{ms}`** 时间桶一致：生产调用处传入当前 [`wall_clock_ms`](../../crates/storage/hc-bootstrap/src/lib.rs)；**单测**用同一毫秒驱动 [`prepare_chat_request_with_swarm_clock`](../../crates/runtime/hc-service/src/chat.rs)；直连 **`/chat`** 时前缀 **`chat.api`**，**[`handle_turn_request`](../../crates/runtime/hc-service/src/turn.rs)** 回落 **ChatFallback** 再走 prepare 时为 **`turn.chat_fallback`**）、**会话槽**写入 **`persist_session_swarm_active_task_binding`**，并在 **`coordination/<slug>.implicit-intent.jsonl`** 上按 ADR-003 对 **`ImplicitIntentDedupeKey::from_trigger(session_id, routing_message_id, user_text)`** 做 **`contains` → `append`**；**`routing_and_binding`** trace 回填 **`active_task_id`**，[`emit_http_chat_create_implicit_task_binding_trace`](../../crates/agent/hc-agent/src/swarm_routing.rs) 携带 **`http_implicit_task_id`** 与 journal 相对路径。**首协同事务化占位**：同轮在隐式 id 下调用 [`ensure_http_implicit_task_plan_stub`](../../crates/agent/hc-agent/src/persistence.rs)（缺省/空 body 时走 [`persist_task_artifacts`](../../crates/agent/hc-agent/src/persistence.rs) + **`TaskPlan::awaiting_planner_input`**；在 **Planning Notes** 写入 **`routing_message_id` / `session_id`**；并 **直接 `work_items.push` 一条 `Planned` 占位项**（[`HTTP_IMPLICIT_WORK_ITEM_HOLDER_ID`](../../crates/agent/hc-agent/src/planning.rs)，不把 plan 标为 **Drafted**）；**planner 在同一 `TaskPlan` 上新增其它 work item 后**，[`persist_task_artifacts`](../../crates/agent/hc-agent/src/persistence.rs) 对 **内存克隆** 先调用 [`prune_http_implicit_work_item_placeholder`](../../crates/agent/hc-agent/src/planning.rs) 再写盘，去掉占位行；**工作台** [`refresh_persisted_task_artifacts`](../../crates/ui/hc-ui/src/lib.rs) 经 [`persist_task_artifacts_with_in_memory_prune`](../../crates/agent/hc-agent/src/persistence.rs) 同步寄存；已有非空正文不覆盖），使 HTTP **L2/L3** 同轮 **`task_plan.md` 节选**可不经人工预置即生效；失败仅 **`tracing::warn`**，不阻塞 chat。**回归**：[`l23_http_implicit_task_anchor_appends_task_plan_excerpt_when_file_preseeded`](../../crates/runtime/hc-service/src/chat.rs)、[`l23_http_implicit_prepare_materializes_task_plan_stub_for_prompt_excerpt`](../../crates/runtime/hc-service/src/chat.rs)、[`http_implicit_task_plan_stub_idempotent_when_body_nonempty`](../../crates/agent/hc-agent/tests/unit/persistence.rs)、[`persist_task_plan_drops_http_implicit_holder_when_planner_adds_work_items`](../../crates/agent/hc-agent/tests/unit/persistence.rs)、[`persist_task_artifacts_with_in_memory_prune_updates_live_plan`](../../crates/agent/hc-agent/tests/unit/persistence.rs)。
- [ ] **隐式工单后续（与 Phase 7 同轨）**：在工作台/HTTP **非退化**语义下跑通 **完整** work item **执行**编排，并以 **ADR-005** 完成 **结构化**对外回灌（与下节 consolidate 收口一致，避免重复描述）。
- [x] **`single_agent_execute` 退化（可选 P0）**：小 L2 可跳过新开 execution agent；**必须在 trace 中标明**，避免与 planner_only 语义混淆。——**HTTP chat**：L2/L3 在 `routing_and_binding` 之后追加 [`emit_http_chat_single_agent_execute_degenerate_trace`](../../crates/agent/hc-agent/src/swarm_routing.rs)（`code`: `http_chat_single_agent_execute_degenerate`，`execution_mode`: `single_llm_route_agent`，含 `selected_agent_id`）；与工作台 `message_nomination_skipped_for_routing_tier` 对称。
...
- [ ] **Speaker + 工件全链 + consolidate 编排**：完整 **L1 直出 / L2+L3 planner 对外 / 多 `execution_result`+`review_note` 必 consolidate** 的 **结构化**编排与 **`artifact_schema_v1`** 挂载仍待收口（含 Phase 5 **隐式工单后续**所列非退化执行与结构化回灌）。**已有切片**：HTTP L2/L3 **退化 MarkDone** 成功时会自动写入 **`task/execution/execution-result.*.json`**（[`persist_execution_result_artifact_v1`](../../crates/agent/hc-agent/src/persistence.rs)、[`ExecutionResultArtifactV1`](../../crates/foundation/hc-protocol/src/swarm.rs)）与 **`task/review/review-note.*.json`**（[`persist_review_note_artifact_v1`](../../crates/agent/hc-agent/src/persistence.rs)、[`ReviewNoteArtifactV1`](../../crates/foundation/hc-protocol/src/swarm.rs)）；HTTP L2/L3 **planner steering** 成功应用草案后会自动写 **`task/plan/plan-note.*.json`**（[`persist_plan_note_artifact_v1`](../../crates/agent/hc-agent/src/persistence.rs)、[`PlanNoteArtifactV1`](../../crates/foundation/hc-protocol/src/swarm.rs)）。同租户 HTTP **`prepare_chat_request`** 在 L2/L3 + task 锚下会按稳定顺序 **`execution_result` → `plan_note` → `review_note`** 将最近摘要**只读**注入 system prompt（对应 **[`load_task_execution_result_artifacts_v1`](../../crates/agent/hc-agent/src/persistence.rs) + [`format_execution_results_digest_for_http_l23`](../../crates/agent/hc-agent/src/persistence.rs)**、**[`load_task_plan_note_artifacts_v1`](../../crates/agent/hc-agent/src/persistence.rs) + [`format_plan_notes_digest_for_http_l23`](../../crates/agent/hc-agent/src/persistence.rs)**、**[`load_task_review_note_artifacts_v1`](../../crates/agent/hc-agent/src/persistence.rs) + [`format_review_notes_digest_for_http_l23`](../../crates/agent/hc-agent/src/persistence.rs)**；非 LLM consolidate）。**planner 对外多源合成 LLM 调用** 仍未接。
- [x] **HTTP L2/L3 发言附录（P0 软约束）**：[`prepare_chat_request`](../../crates/runtime/hc-service/src/chat.rs) 对 **L2/L3** 在 **可选**、**只读** `task_plan.md` 节选之后追加 **ADR-005 Outward speaker** 短文（**planner** 对外身份、多源时简要 synthesize）；**不构成** consolidate 闭环或结构化工件编排。
...
## Part A：仍未勾选条目速览（截至文档版本）

| 章节 | 条目 | 性质 |
| --- | --- | --- |
| Phase 5 | **`L2/L3`（HTTP chat 对等）** | HTTP 侧完整编排 vs `route_agent` 单次生成 |
| Phase 5 | **隐式工单后续** | 非退化 work item 执行 + ADR-005 结构化回灌 |
| Phase 7 | **Speaker + 工件全链 + consolidate** | 已有 `plan/execution/review` 三类工件落盘能力 + prompt 只读注入；多源 consolidate 与结构化挂载仍待收口 |
| Part B | **B-0～B-6 全部** | ADR-006 经验副链（与主链解耦） |

---

## Part B：Experience lane（ADR-006，P1 / P1.5 / P2）

**硬性约束**：与 [ADR-006](../adr/ADR-006-experience-lane-boundary.md) 一致——**任务主链结束不依赖副链成功**；`L1` 不跑 playbook evolution；reflection / curator 严于 raw outcome（须 **租户允许 + task 授权 `enable_learning` + 通常高风险档位**）。**本节在 P0 阶段保持大量 `[ ]` 为正常状态**；勿与 Part A 的 HTTP/consolidate 缺口混为同一迭代。

### B-0 策略与协议打底

- [x] **策略模型（门控骨架）**：**`TaskRequest.enable_learning`**（默认 `false`，`with_enable_learning`）+ 部署级 **`HC_TENANT_LEARNING_ALLOWED`**（`1`/`true`/`on`…）+ [`task_learning_effective`](../../crates/agent/hc-agent/src/experience_lane.rs)（`hc-agent::experience_lane`）。多租户配置与 conversation 侧「仅提议不可越权」仍属后续。
- [ ] **策略模型（完整）**：tenant 策略存储、conversation 仅可提议不可越权（ADR-006 §2）与 API 暴露。
...
| **M3** | Phase 5–6：HTTP L2/L3 **隐式工单**、**节选**、`maybe_persist_http_chat_l23_*` **退化** claim/assign、`single_agent_execute` trace、Assign 胜者规则；**完整**端到端编排与结构化对外发言仍收口于 Phase 5/7 |
