...
- [ ] **Speaker + 工件全链 + consolidate 编排**：完整 **L1 直出 / L2+L3 planner 对外 / 多 `execution_result`+`review_note` 必 consolidate** 的 **结构化**编排与 **`artifact_schema_v1`** 挂载仍待收口（含 Phase 5 **隐式工单后续**所列非退化执行与结构化回灌）。**已有切片**：HTTP L2/L3 **退化 MarkDone** 成功时会自动写入 **`task/execution/execution-result.*.json`**（[`persist_execution_result_artifact_v1`](../../crates/hc-agent/src/persistence.rs)、[`ExecutionResultArtifactV1`](../../crates/hc-protocol/src/swarm.rs)）与 **`task/review/review-note.*.json`**（[`persist_review_note_artifact_v1`](../../crates/hc-agent/src/persistence.rs)、[`ReviewNoteArtifactV1`](../../crates/hc-protocol/src/swarm.rs)）；HTTP L2/L3 **planner steering** 成功应用草案后会自动写 **`task/plan/plan-note.*.json`**（[`persist_plan_note_artifact_v1`](../../crates/hc-agent/src/persistence.rs)、[`PlanNoteArtifactV1`](../../crates/hc-protocol/src/swarm.rs)）。同租户 HTTP **`prepare_chat_request`** 在 L2/L3 + task 锚下会按稳定顺序 **`execution_result` → `plan_note` → `review_note`** 将最近摘要**只读**注入 system prompt（对应 **[`load_task_execution_result_artifacts_v1`](../../crates/hc-agent/src/persistence.rs) + [`format_execution_results_digest_for_http_l23`](../../crates/hc-agent/src/persistence.rs)**、**[`load_task_plan_note_artifacts_v1`](../../crates/hc-agent/src/persistence.rs) + [`format_plan_notes_digest_for_http_l23`](../../crates/hc-agent/src/persistence.rs)**、**[`load_task_review_note_artifacts_v1`](../../crates/hc-agent/src/persistence.rs) + [`format_review_notes_digest_for_http_l23`](../../crates/hc-agent/src/persistence.rs)**；非 LLM consolidate）。**planner 对外多源合成 LLM 调用** 仍未接。
...
- [x] **策略模型（门控骨架）**：**`TaskRequest.enable_learning`**（默认 `false`，`with_enable_learning`）+ 部署级 **`HC_TENANT_LEARNING_ALLOWED`**（`1`/`true`/`on`…）+ [`task_learning_effective`](../../crates/hc-agent/src/experience_lane.rs)（`hc-agent::experience_lane`）。多租户配置与 conversation 侧「仅提议不可越权」仍属后续。
